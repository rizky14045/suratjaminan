                ...
                <div class="col-xl-3 col-lg-4 col-md-12 theiaStickySidebar">
                    <aside class="sidebar sidebar-user">
                        ...
                        <div class="quicklink-sidebar-menu ctm-border-radius shadow-sm bg-white card">
                            <div class="card-body">
                                <ul class="list-group">
                                    <li class="list-group-item text-center button-1"><a href="{{ url('/mkad/surat-keterangan') }}"
                                                ...
                                                <tbody>
                                                    @foreach ($visas as $i => $item)
                                                        <tr class={{$item->rangking == 2 ? 'bg-danger' :''}}>
                                                            ...
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->jenis }}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->negara_tujuan }}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->keperluan }}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->tujuan }}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->alamat }}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->tanggal_mulai  }} s/d {{$item->tanggal_selesai}}</td>
                                                            <td class="text-ck {{$item->rangking == 2 ? 'text-white' :''}}">{{ $item->status  }}</td>

                                                            <td class="text-right" align="center">
                                                                <div class="table-action">
                                                                    <a href="{{ url('/mkad/visa/pdf/' . $item->id) }}"
                                                                        title="View visa"><button
                                                                            class="btn btn-sm btn-info"><span
                                                                                class="lnr lnr-eye"></span>PDF</button></a>
                                                                        @if ($item->rangking == 2)
                                                                        <a href="{{ url('/mkad/visa/approve/' . $item->id ) }}"
